...
class Viewer:
    ...
    def is_enabled_menu(self, value):
        ...
        if value and self.menu.main_screen is None:
            self.set_menu()
        self._is_enabled_menu = value

    def set_menu(self):
        """Build and install the entire menu tree into ``self.menu``.

        Tree structure
        --------------
        MENU
        ├── MODE          (enum: AUTO / MAINTENANCE / STAND BY)
        ├── RELAYS
        │   ├── LIGHTS    (toggle relay 0)
        │   ├── FILTER    (toggle relay 1)
        │   ├── HEATER    (toggle relay 2)
        │   ├── FEEDER    (toggle relay 3)
        │   └── <<< BACK
        ├── SENSORS
        │   ├── EC
        │   │   ├── ACTIVATION  (toggle)
        │   │   ├── MONITORING  (live sensor, visible when EC is active)
        │   │   ├── WEB SERVER  (toggle sending, visible when EC is active)
        │   │   ├── WEB RATE    (enum, visible when sending is active)
        │   │   ├── SEND TO WEB (confirm, visible when sending is active)
        │   │   └── <<< BACK
        │   ├── PH          (same structure as EC)
        │   ├── THERMOMETER (toggle + web sending)
        │   └── <<< BACK
        └── SETTINGS
            ├── WIFI         (info + reconnect confirm)
            ├── DATE/TIME    (interactive editor)
            ├── LIGHT TIMER  (ON/OFF time setting)
            ├── HEATER AUTO  (toggle + temperature thresholds)
            ├── FILTER AUTO  (toggle + frequency enum)
            ├── RECOVERY     (confirm)
            └── <<< BACK
        """
        self.menu.set_main_screen(
            ...
            .add(
                MenuEnum(
                    self.display,
                    "MODE",
                    self._config.mode_list,
                    (self._config.set_mode),
                )
            )
            .add(
                MenuList(self.display, "RELAYS")
                .add(
                    ToggleItem(
                        "LIGHTS",
                        (self._config.get_on_off_light_auto),
                        (self.toggle_on_off_light_auto),
                        ("ON", "OFF"),
                    )
                )
                .add(
            ...
            .add(
                MenuList(self.display, "SENSORS")
                .add(
                    MenuList(self.display, "EC")
                    .add(
                        ToggleItem(
                            "ACTIVATION",
                            (self._config.get_on_off_ec),
                            (self.toggle_on_off_ec),
                        )
                    )
                    .add(
                        MenuMonitoringSensor(
                            self.display,
                            "MONITORING",
                            visible=(self._config.get_on_off_ec),
                        )
                    )
                    .add(
                        ToggleItem(
                    ...
                    .add(
                        MenuEnum(
                            ...
                            "WEB RATE",
                            self._config.freq,
                            self._config.set_freq_update_web_ec,
                            visible=(self._config.get_on_off_ec_sending),
                        )
                    )
                    .add(
                        MenuConfirm(
                ...
                .add(
                    MenuList(self.display, "PH")
                    .add(
                        ToggleItem(
                            "ACTIVATION",
                            (self._config.get_on_off_ph),
                            (self.toggle_on_off_ph),
                        )
                    )
                    .add(
                        MenuMonitoringSensor(
                            self.display,
                            "MONITORING",
                            visible=(self._config.get_on_off_ph),
                        )
                    )
                    .add(
                        ToggleItem(
                    ...
                    .add(
                        MenuEnum(
                            ...
                            "WEB RATE",
                            self._config.freq,
                            self._config.set_freq_update_web_ph,
                            visible=(self._config.get_on_off_ph_sending),
                        )
                    )
                    .add(
                        MenuConfirm(
                ...
                .add(
                    MenuList(self.display, "THERMOMETER")
                    .add(
                        ToggleItem(
                            "ACTIVATION",
                            (self._config.get_on_off_temperature),
                            (self.toggle_on_off_temperature),
                        )
                    )
                    .add(
                        ToggleItem(
                            ...
                            (self.toggle_on_off_temperature_sending),
                            visible=(self._config.get_on_off_temperature),
                        )
                    )
                    .add(
                        MenuEnum(
                            ...
                            "WEB RATE",
                            self._config.freq,
                            self._config.set_freq_update_web_temperature,
                            visible=(self._config.get_on_off_temperature_sending),
                        )
                    )
                    .add(
                        MenuConfirm(
            ...
            .add(
                MenuList(self.display, "SETTINGS")
                .add(
                    MenuList(self.display, "WIFI")
                    ...
                    .add(
                        MenuConfirm(
                            ...
                            "CONNECTING",
                            ("-> YES", "<- NO"),
                            self._config.set_connection_action,
                        )
                    )
                    .add(BackItem())
                )
                ...
                .add(
                    MenuSetTimer(
                        self.display,
                        "LIGHT TIMER",
                        self._config.get_timer_time(),
                        self._config.set_timer_time,
                    )
                )
                .add(
                    MenuList(self.display, "HEATER AUTO")
                    .add(
                        ToggleItem(
                            "ACTIVATION",
                            (self._config.get_on_off_heater_auto),
                            (self.toggle_on_off_heater_auto),
                        )
                    )
                    .add(
                        MenuHeaterManage(
                            ...
                            "SETTING",
                            self._config.get_timer_time(),
                            self._config.set_auto_heater,
                            visible=(self._config.get_on_off_heater_auto),
                        )
                    )
                    .add(BackItem())
                )
                .add(
                    MenuList(self.display, "FILTER AUTO")
                    .add(
                        ToggleItem(
                            "ACTIVATION",
                            (self._config.get_on_off_filter_auto),
                            (self.toggle_on_off_filter_auto),
                        )
                    )
                    .add(
                        MenuEnum(
                            ...
                            "RATE",
                            self._config.freq,
                            self._config.set_freq_filter,
                            visible=(self._config.get_on_off_filter_auto),
                        )
                    )
                    .add(BackItem())
                )
                .add(
                    MenuConfirm(
                        ...
                        "RECOVERY",
                        ("-> YES", "<- NO"),
                        self._config.set_on_off_recovery,
                    )
                )
                .add(BackItem())
            )
