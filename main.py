...
def main():
    ...
    # Se l'oscillatore si è fermato (es. batteria tampone scarica),
    # sincronizza l'ora via NTP per garantirne l'accuratezza.
    if rtc.OSF():
        print("RTC oscillator stopped. Syncing time with NTP...")
        from Manager.ntpManager import NTP
        ntp = NTP(wifi, rtc)
        ntp.sync()
        del ntp # Libera memoria
    ...
    if sdm:
        if sdm.if_exist_configuration():
            ...
            sdm.set_configuration(cfg.to_dict())
    '''
        ...
        try:
            ...
            if key == board.up_keypad_value:
                ...
                if not viewer.is_enabled_menu:
                    ...
                    countdown_menu = 0
                    viewer.menu.click()
            elif key == board.right_keypad_value:
                if viewer.is_enabled_menu:
                    countdown_menu = 0
                    viewer.menu.shift(1)
            ...
            lightsleep(200)  # consumo ridotto ~80 mA a ~20 mA
...
def _create_menu(viewer, cfg: Config):
