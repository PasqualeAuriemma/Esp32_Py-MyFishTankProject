$(document).ready(function() {
  $('#tempTableSetting').DataTable({
    ...
    'order':[],
    'ajax': {
      'url':'php/TEMPSettingTable/fetch_temp_settings.php',
      'type':'post'},
    "columnDefs": [{
      'targets': '_all',
      'orderable':false,
...
$(document).on('click','.deleteBtnTempS',function(event){
  ...
  if(confirm("Are you sure want to delete this value?")){
    $.ajax({
      url:"php/TEMPSettingTable/delete_temp_settings.php",
      data:{id:id},
      type:"post",
...
$(document).on('click','.editbtnConfirmTempS',function(event){
    ...
    $row.find('td').not(':first').not(':last').not(':nth-child(4)').each(function(i, el) {
     ...
     $.ajax({
      url:"php/TEMPSettingTable/update_temp_settings.php",
      type:"post",
      data:{temp:rowList[0], data_send:rowList[1], id:id},
      success:function(data){   
        var json = JSON.parse(data);
